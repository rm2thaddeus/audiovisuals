...
class CPPN(nn.Module):
    ...
    Hidden: [batch, 256] × 8-12 layers with mixed activations
    Output: [batch, 3]  # RGB
```
...
**Optimization**:
- Batch size tuning for RTX 5070 (8GB VRAM)
- Half-precision (FP16) if needed for speed
...
## Success Criteria (Phase A: Baseline)

- [ ] Audio Analyzer extracts FFT features @ 60 Hz
- [ ] CPPN network initializes on CUDA (RTX 5070)
- [ ] Renderer generates 720p frames without errors
- [ ] Video Encoder produces synced MP4 output
- [ ] CLI runs end-to-end: audio file → video file
- [ ] Performance: <5min for 3min audio track
